...
def tanh_grad_descent(x,y,i):
    ...
    for k in range(max_its):
        ...
        for p in np.arange(0,P):
            ...
            q[p] = b+ np.sum((np.tanh(x[p][0]*v + c)*w), axis=0) - y[p][0]

        # YOUR CODE HERE, for grad_b
        one_arr = l_P.T    
        grad_b = 2*np.dot(one_arr, q) 
        ...
        for m in np.arange(0,M): 
            # YOUR CODE HERE
            t = np.tanh(c[m][0]+x*v[m][0])
            s = (1/np.cosh(c[m][0]+x*v[m][0]))**2 
            grad_w[m] = 2*np.dot(one_arr,(q*t))
            grad_c[m] = 2*np.dot(one_arr,((q*s)*w[m][0]))
            grad_v[m] = 2*np.dot(one_arr,((q*x*s)*w[m][0]))
